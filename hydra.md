...
Внутри каждой группы (папки) можно поместить произвольное число `.yaml`-файлов, соответствующих различным вариантам настроек. Добавим набор параметров для простой предобработки текстовых данных (`preprocessing/rubert_minimal.yaml`), параметры для инициализации модели RuBert-tiny (`model/rubert.yaml`), а также параметры обучения модели (`rubert/training.yaml`), так что итоговое дерево файлов будет выглядеть следующим образом:
